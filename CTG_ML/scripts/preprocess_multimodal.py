...
from ctg_ml.multimodal_preprocess import build_multimodal_npz_files


def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Preprocess CTG parquet + registry CSV into multimodal NPZ files for TCN training."
        )
    )
    ...
    if not splits_path.exists():
        raise FileNotFoundError(
            f"Missing splits file: {splits_path}. Run "
            f"`uv run python scripts/make_splits_multimodal.py --config {args.config}` first."
        )

    print(
        f"Building multimodal NPZ: window={cfg.sequence.window_minutes} min, "
        f"steps={cfg.sequence.window_minutes * 60 * cfg.sequence.sample_rate_hz}, "
        f"pad_short={cfg.sequence.pad_short}, "
        f"quality_channels={cfg.sequence.include_signal_quality_channels}, "
    ...
    stats = build_multimodal_npz_files(
    ...
    for s in stats:
        print(
            ...
            f"dropped_short={s.dropped_short_babies} "
