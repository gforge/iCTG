...
def compute_apgar_class_weights(
    ...
    for idx in range(num_targets):
        ...
        # Keep the average near 1.0 so this does not silently rescale the full loss.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train a multimodal multitask TCN on multimodal NPZ files."
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Registry-only ablation: replace all CTG sequence inputs with zeros "
            "after normalization."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "CTG-only ablation: replace all registry/tabular inputs with the "
            "train-set mean feature vector."
        ),
    )
    parser.add_argument(
        ...
        default=None,
        help=(
            "Comma-separated raw registry input columns to ablate by replacing "
            "their encoded features with the train-set mean values."
        ),
    )
    parser.add_argument(
        "--seed-override",
    ...
    if not train_npz.exists() or not val_npz.exists() or not test_npz.exists():
        raise FileNotFoundError(
            "Missing multimodal NPZ files. Run "
            "`uv run python scripts/preprocess_multimodal.py` first."
        )
    ...
    print(
        ...
        f"gradient_clip_norm={cfg.train.gradient_clip_norm}, "
        f"continuous_weight={cfg.train.regression_loss_weight}, "
        f"binary_weight={cfg.train.binary_loss_weight}"
    )
    if cfg.train.early_stopping_enabled:
        print(
            "Early stopping: enabled "
            f"(min_epochs={cfg.train.early_stopping_min_epochs}, "
    ...
    for epoch in range(1, cfg.train.epochs + 1):
        ...
        if monitor > (best_monitor_for_stop + cfg.train.early_stopping_min_delta):
            ...
            epochs_since_improve = 0
            print(
                "Early stopping monitor: significant monitor_binary_PR-AUC "
                f"improvement (best={best_monitor_for_stop:.4f})"
            )
        else:
