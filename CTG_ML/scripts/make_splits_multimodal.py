...
from ctg_ml.data import print_label_summary
from ctg_ml.multimodal_config import load_multimodal_config
from ctg_ml.multimodal_registry import load_registry_labels_multimodal
from ctg_ml.splits import SplitFractions, create_stratified_splits, print_split_summary, save_splits


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create BabyID-level train/val/test splits for multimodal CTG training."
    )
    parser.add_argument("--config", default="configs/ctg3_multimodal.toml")
    parser.add_argument(
        "--out", default=None, help="Output CSV path (defaults to <artifacts_dir>/splits.csv)"
    )
    ...
    labels = load_registry_labels_multimodal(
