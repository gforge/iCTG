...
def _fmt_summary_drop(row: pd.Series, outcome: str, metric: str) -> str:
...
def _build_readable_markdown(
    ...
    lines = [
        ...
        "- Main comparison metric remains ROC-AUC. PR-AUC is included as a second view "
        "for rare outcomes.",
        "",
        "## Compact ROC-AUC Table",
        "",
        "| Ablation | Apgar mean | Apgar1 | Apgar5 | Apgar10 | pH<7 | Shoulder dystocia | "
        "Hypoglycemia | Sepsis/pneumonia | Neonatal anemia | Respirator |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    ...
    for outcome in SUMMARY_OUTCOMES:
        ...
        for i, (_, row) in enumerate(ranked.iterrows(), start=1):
            roc_drop = _fmt_value(
                row[f"{outcome}_mean_drop_roc_auc"],
                row[f"{outcome}_sd_drop_roc_auc"],
            )
            pr_drop = _fmt_value(
                row[f"{outcome}_mean_drop_pr_auc"],
                row[f"{outcome}_sd_drop_pr_auc"],
            )
            lines.append(
    ...
    lines.extend(["## Notes", ""])
    lines.append(
        "- Negative values mean the ablated run did slightly better than the baseline "
        "on that metric."
    )
    lines.append(
        "- With one seed, all standard deviations are 0 and small negative values are "
        "often just training noise."
    )
    return "\n".join(lines)
...
def run_study(
    ...
    train_script = Path(__file__).with_name("train_multimodal_tcn.py")
    ...
    md_lines = [
        "# Multimodal Ablation Study Summary",
        "",
        f"- Config: `{config_path}`",
        f"- Seeds: {', '.join(str(s) for s in seeds)}",
        f"- Mode: {mode}",
        ...
        "",
        "| " + " | ".join(SUMMARY_TABLE_LABELS) + " |",
        "|---" + "|---:" * (len(SUMMARY_TABLE_LABELS) - 1) + "|",
    ]
    for _, row in summary_df.iterrows():
        values = [
            str(row["ablation"]),
            *[_fmt_summary_drop(row, outcome, "roc_auc") for outcome in SUMMARY_OUTCOMES],
        ]
    ...
    for _, row in summary_df.iterrows():
        values = [
            str(row["ablation"]),
            *[_fmt_summary_drop(row, outcome, "pr_auc") for outcome in SUMMARY_OUTCOMES],
        ]
        md_lines.append("| " + " | ".join(values) + " |")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run automated grouped/single ablation study for multimodal training."
    )
