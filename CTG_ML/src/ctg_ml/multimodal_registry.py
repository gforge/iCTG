...
def load_registry_for_multimodal(
    ...
    for col in (
        registry_cfg.input_numeric + registry_cfg.apgar_outputs + registry_cfg.continuous_outputs
    ):
...
def transform_tabular_inputs(df: pd.DataFrame, encoder: TabularEncoder) -> np.ndarray:
    ...
    if cursor != features.shape[1]:
        raise RuntimeError(
            f"Tabular feature assembly mismatch: cursor={cursor}, width={features.shape[1]}"
        )
    return features
