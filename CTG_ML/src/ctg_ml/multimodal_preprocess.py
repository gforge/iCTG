...
class MultimodalSplitBuildStats:
    split_name: str
    total_babies: int
    kept_babies: int
...
def _build_split_npz(
    ...
    return MultimodalSplitBuildStats(
...
def build_multimodal_npz_files(
    ...
    for split_name in ["train", "val", "test"]:
        split_part = (
            merged[merged["split"] == split_name]
            .copy()
            .sort_values("BabyID")
            .reset_index(drop=True)
        )
        X_tab = transform_tabular_inputs(split_part, encoder)
        y_apgar, y_apgar_mask, y_cat, y_cat_mask, y_reg, y_reg_mask, y_bin, y_bin_mask = (
            build_targets(split_part, target_spec)
        )
    ...
    stats: list[MultimodalSplitBuildStats] = []
    categorical_class_counts = [
        len(target_spec.categorical_levels[name]) for name in target_spec.categorical_names
    ]
    for split_name in ["train", "val", "test"]:
        (
            X_tab,
